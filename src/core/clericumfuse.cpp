...
bool ClericumFuse::mount() {
    if (storePath().isEmpty()) {
        critical(i18n("Store path not set"));
        return false;
    }

    if (m_mountPath.isEmpty()) {
        critical(i18n("Mount path not set"));
        return false;
    }

    if (!m_storeManager->isValidStore()) {
        critical(i18n("Invalid store %1", storePath()));
        return false;
    }
    ...
    if (!mountDir.exists()) {
        if (!mountDir.mkpath(".")) {
            critical(i18n("Failed to create mount directory %1", m_mountPath));
            return false;
        }
    }
    ...
    if (ret != 0) {
        critical(i18n("FUSE exited with code %1", ret));
        return false;
    }
